...
$months_till_now = mysqli_query($conn, "SELECT DISTINCT month FROM `orders`");
      ...
      <aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
        <div class="app-brand demo">
          <a href="index.html" class="app-brand-link">
            <span class="app-brand-text demo menu-text fw-bolder ms-2">BVRMOL</span>
          </a>
        </div>
        ...
        <div class="content-wrapper">
          ...
          <div class="container-xxl flex-grow-1 container-p-y">
            ...
            while($month = mysqli_fetch_assoc($months_till_now)){
              $month = $month['month'];
              $total_margin = mysqli_fetch_assoc(mysqli_query($conn, "SELECT SUM(margin) as total_margin FROM `orders` WHERE month = '$month'"))['total_margin'];

              $total_orders = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as total_orders FROM `orders` WHERE month = '$month'"))['total_orders'];

              $average_margin = $total_margin / $total_orders;
            ?>
            <div class="card">
              <h5 class="card-header"><?php echo $month; ?> Month</h5>
              ...
              <div class="table-responsive text-nowrap">
                <table class="table">
                  ...
                  <tbody class="table-border-bottom-0">
                    ...
                    $sellers = mysqli_query($conn, "SELECT DISTINCT seller FROM `orders` WHERE month = '$month'");
                    while($seller = mysqli_fetch_assoc($sellers)){
                      $seller = $seller['seller'];
                      $month_total_margin = mysqli_fetch_assoc(mysqli_query($conn, "SELECT SUM(margin) as total_margin FROM `orders` WHERE month = '$month' AND seller = '$seller'"))['total_margin'];

                      $month_total_orders = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as total_orders FROM `orders` WHERE month = '$month' AND seller = '$seller'"))['total_orders'];
